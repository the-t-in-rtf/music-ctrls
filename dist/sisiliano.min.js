...
!function(a, b, c) {
    ...
    b.defaults("fluid." + a.frameworkName + ".piano", {
    ...
    }), b[a.frameworkName].piano.onChange = function() {
        console.log("onChange");
    }, b[a.frameworkName].piano.onCreate = function(c) {
        b[a.frameworkName].piano.generateKeyboard(c), b[a.frameworkName].piano.appendListeners(c), 
        b[a.frameworkName].piano.moveTabBy(c, 0), b[a.frameworkName].piano.refresh(c), b[a.frameworkName].piano.updateTheDisabledAreaUI(c);
    }, b[a.frameworkName].piano.refresh = function(c) {
        b[a.frameworkName].util.getTemplate(function(a) {
            var b = a(c.model);
            c.container.html(b);
        }, "src/templates/piano.html");
    }, b[a.frameworkName].piano.updateTheDisabledAreaUI = function(c) {
        for (var d = c.container.find(".fl-sisiliano-piano-key"), e = 0; e < d.length; e++) b[a.frameworkName].util.removeClass(d[e], "fl-sisiliano-piano-key-pressed");
        var f = c.container.find(".fl-sisiliano-piano-white-key");
        for (e = 0; e < f.length; e++) e >= c.model.activeArea.start && e <= c.model.activeArea.end ? b[a.frameworkName].util.removeClass(f[e], "fl-sisiliano-piano-key-inactive") : b[a.frameworkName].util.addClass(f[e], "fl-sisiliano-piano-key-inactive");
        ...
        c.model.disabledArea.left.width = c.model.activeArea.start * c.model.keyBoard.whiteKey.width, 
        c.model.disabledArea.right.width = (c.model.keyBoard.whiteKeys.length - c.model.activeArea.end) * c.model.keyBoard.whiteKey.width, 
        c.model.activeArea.end < c.model.keyBoard.whiteKeys.length && (c.model.disabledArea.right.x = c.model.keyBoard.whiteKeys[c.model.activeArea.end].x + c.model.keyBoard.whiteKey.width);
        ...
        masterGain = f.createGain(), nodes = {}, masterGain.gain.value = .3, masterGain.connect(f.destination);
        var g = [ 261.626, 277.183, 293.665, 311.127, 329.628, 349.228, 369.994, 391.995, 415.305, 440, 466.164, 493.883 ], h = function(a, b) {
            return Math.pow(2, a) * g[b];
        };
        c.container.on("keydown", function(g) {
            var i = g.keyCode, j = e[i];
            if (j) {
                if ("fl-sisiliano-piano-key-pressed" !== j.statusClass) {
                    j.statusClass = "fl-sisiliano-piano-key-pressed", d(c, j);
                    var k = f.createOscillator();
                    k.type = "square", k.frequency.value = h(j.octave, j.octaveIndex), console.log("mappedPianoKey.octave : " + j.octave + " -- mappedPianoKey.octaveIndex : " + j.octaveIndex), 
                    k.connect(masterGain), k.start(0), nodes[j.index] = k;
                }
            } else console.log("keydown : not found");
            switch (i) {
              case 37:
                return b[a.frameworkName].piano.moveTabBy(c, -1), b[a.frameworkName].piano.updateTheDisabledAreaUI(c), 
                !1;
            ...
            if (f) {
                if ("fl-sisiliano-piano-key-pressed" === f.statusClass) {
                    f.statusClass = null, d(c, f);
                    var g = nodes[f.index];
                    nodes[f.index] = null, g.stop(0), g.disconnect();
                }
            } else console.log("keyup : not found");
        });
    ...
    b[a.frameworkName].piano.generateKeyboard = function(c) {
        c.model.keyBoard.blackKey.width = (c.model.keyBoard.whiteKey.width - 1) / 3 * 2 + 1, 
        c.model.keyBoard.blackKey.height = c.model.keyBoard.whiteKey.height / 3 * 2, c.model.keyBoard.keyCodeListenerMap = {}, 
        c.model.keyBoard.whiteKeys = [], c.model.keyBoard.blackKeys = [];
        ...
        for (var e = c.model.keyBoard.start; e < c.model.keyBoard.length + c.model.keyBoard.start; e++) {
            ...
            "WHITE" === b[a.frameworkName].piano.OCTAVE[g].color ? (f = {
                x: c.model.keyBoard.padding.left + c.model.keyBoard.whiteKeys.length * c.model.keyBoard.whiteKey.width,
                y: c.model.keyBoard.padding.top,
                width: c.model.keyBoard.whiteKey.width - 1,
                height: c.model.keyBoard.whiteKey.height,
                index: e,
                octave: Math.floor(e / 12),
                octaveIndex: g,
                "class": "fl-sisiliano-piano-key fl-sisiliano-piano-white-key"
            }, c.model.keyBoard.whiteKeys.push(f)) : "BLACK" === b[a.frameworkName].piano.OCTAVE[g].color && (f = {
                x: c.model.keyBoard.padding.left + (c.model.keyBoard.whiteKeys.length - 1) * c.model.keyBoard.whiteKey.width + c.model.keyBoard.whiteKey.width / 3 * 2,
                y: c.model.keyBoard.padding.top,
                width: c.model.keyBoard.blackKey.width,
                height: c.model.keyBoard.blackKey.height,
                index: e,
                octave: Math.floor(e / 12),
                octaveIndex: g,
                "class": "fl-sisiliano-piano-key fl-sisiliano-piano-black-key"
        ...
        c.model.viewBox.width = c.model.keyBoard.whiteKeys.length * c.model.keyBoard.whiteKey.width + c.model.keyBoard.padding.left + c.model.keyBoard.padding.right, 
        c.model.viewBox.height = c.model.keyBoard.whiteKey.height + c.model.keyBoard.padding.top + c.model.keyBoard.padding.bottom;
    ...
    }, b[a.frameworkName].util.addClass = function(d, e) {
        b[a.frameworkName].util.removeClass(d, e);
        var f = c(d).attr("class"), g = f + " " + e;
        c(d).attr("class", g);
    }, b[a.frameworkName].util.removeClass = function(a, b) {
        var d = c(a).attr("class"), e = d.replace(new RegExp(" " + b, "g"), "").replace(new RegExp(b, "g"), "");
        c(a).attr("class", e);
    };
}(config, fluid, $);